...
namespace Modules\Admin\Services\choiceness;


use Modules\Admin\Models\AnnexeHot;
use Modules\Admin\Models\AnnexeHotData;
use Modules\Admin\Models\Choiceness;
use Modules\Admin\Models\ChoicenessData;
use Modules\Admin\Services\BaseAdminService;

class ChoicenessService extends BaseAdminService
{
    ...
    public function add(array $data)
    {
        ...
        try{
            ...
            $id = Choiceness::query()->insertGetId($data);
            foreach ($info as &$v){
                unset($v['id']);
                $v['choiceness_id'] = $id;
                $v['created_at'] = date('Y-m-d H:i:s');
            }
            ChoicenessData::query()->insert($info);
            \DB::commit();
    ...
    {
        return Choiceness::with('info')->find($id);
    }
    ...
    public function update(int $id, array $data)
    {
        ...
        try{
            ...
            Choiceness::query()->where('id',$id)->update($data);
            foreach ($info as $v){
                $id = $v['id'];
                unset($v['id']);
                $data['updated_at'] = date('Y-m-d H:i:s');
                ChoicenessData::query()->where('id',$id)->update($v);
            }
            \DB::commit();
    ...
    public function del(array $ids)
    {
        ...
        $res = $this->commonDestroy(Choiceness::query(), $ids);
        ChoicenessData::where('choiceness_id', $ids[0])->delete();
        return $res;
