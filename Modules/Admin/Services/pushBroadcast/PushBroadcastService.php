...
namespace Modules\Admin\Services\PushBroadcast;

use Modules\Admin\Models\PushBroadcast;
use Modules\Admin\Models\StoreBranch;
use Modules\Admin\Services\BaseAdminService;

class PushBroadcastService extends BaseAdminService
{
    /**
     * @name 發票捐贈列表
     * @description
    ...
    {
         $data['receive_object'] = json_encode($data['receive_object']);
         if ($id > 0) {
             return $this->commonUpdate(PushBroadcast::query(), $id, $data);
         }
         return $this->commonCreate(PushBroadcast::query(), $data);
    }
    ...
    {
        return $this->commonDestroy(PushBroadcast::query(), $ids);
    }
    /**
     * @name 調整狀態
     * @description
    ...
    {
        return $this->commonStatusUpdate(PushBroadcast::query(), $id, $data);
    }
