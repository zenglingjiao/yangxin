...
namespace Modules\Admin\Services\seasonRecommend;


use Modules\Admin\Models\AnnexeHot;
use Modules\Admin\Models\AnnexeHotData;
use Modules\Admin\Models\SeasonRecommend;
use Modules\Admin\Models\SeasonRecommendData;
use Modules\Admin\Services\BaseAdminService;

class SeasonRecommendService extends BaseAdminService
{
    ...
    public function add(array $data)
    {
        ...
        try{
            ...
            $id = SeasonRecommend::query()->insertGetId($data);
            foreach ($info as &$v){
                unset($v['id']);
                $v['recommend_id'] = $id;
                $v['created_at'] = date('Y-m-d H:i:s');
            }
            SeasonRecommendData::query()->insert($info);
            \DB::commit();
    ...
    {
        return SeasonRecommend::with('info')->find($id);
    }
    ...
    public function update(int $id, array $data)
    {
        ...
        try{
            ...
            SeasonRecommend::query()->where('id',$id)->update($data);
            foreach ($info as $v){
                $id = $v['id'];
                unset($v['id']);
                $data['updated_at'] = date('Y-m-d H:i:s');
                SeasonRecommendData::query()->where('id',$id)->update($v);
            }
            \DB::commit();
    ...
    public function del(array $ids)
    {
        ...
        $res = $this->commonDestroy(SeasonRecommend::query(), $ids);
        SeasonRecommendData::where('recommend_id', $ids[0])->delete();
        return $res;
