...
@include('admin::partials._include_header_aside')
    ...
    <div class="page-content-wrapper">
                    ...
                    <div class="table-responsive">
                        <div class="my-el-table">
                            <el-table :data="table_data" stripe highlight-current-row border
                                ...
                                <el-table-column header-align="center" align="center" sortable="custom" prop="brand_info.zh_name" label="品牌中文名稱"></el-table-column>
                                <el-table-column header-align="center" align="center" sortable="custom" prop="brand_info.en_name" label="品牌英文名稱"></el-table-column>
        ...
        <el-dialog title="編輯" :visible.sync="dialogFormVisible">
            <el-form :model="form">
                <el-form-item label="品牌" prop="weight" :label-width="formLabelWidth">
                    <el-select v-model="form.brand_id" filterable >
                        <el-option v-for="item in options" :label=item.zh_name :value=item.id></el-option>
                    </el-select>
                </el-form-item>
...
<script>
    ...
    var vue_obj = new Vue({
        ...
        data: {
            ...
            loading: false,
            clean_model:{},
            history_name:"brand_recommend_list",

            api_get_tabel:"{{route('brand_recommend_list')}}",
            api_model_delete:"{{route('brand_recommend_del')}}",
            api_model_update:"{{route('brand_recommend_update')}}",
            api_get_select : "{{route('get_brand_select')}}"
        },
        created:function () {
        ...
        methods: {
            ...
            edit_model: function (index, row) {
                ...
                this.form.id = row.id
                this.form.brand_id = row.brand_id
            ...
            get_select_data: function () {
                this.loading = true;
                var that = this;
                request_ajax_json(this.api_get_select, '', function (response) {
                    if (response.status) {
                        switch (response.status) {
                            case 20000:
                                that.loading = false;
