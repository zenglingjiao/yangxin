                ...
                <div class="card-body d-flex flex-wrap">
                    <div class="list-item">
                        <div class="list-item-box">
                            <input type="text" class="form-control" v-model="model.name" autocomplete="off" placeholder="分類名稱/分類編號"/>
                        </div>
                    </div>
                    <div class="list-item">
                ...
                <div class="card-body">
                    <div class="form-item">
                        <div class="form-group ml-auto">
                            <a href="{{route('classify_add')}}?pid={{$parent?$parent->id:0}}" class="btn btn-info m-1 px-5 radius-30">新增</a>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <div class="my-el-table">
                            <el-table :data="table_data" stripe highlight-current-row border
                                ...
                                <el-table-column header-align="center" align="center"  prop="number" label="分類編號"></el-table-column>
                                ...
                                <el-table-column header-align="center" align="center"  prop="status" label="狀態">
                                    <template slot-scope="scope">
                                       <span v-if="scope.row.status == 0">關閉</span>
                                       <span v-else>開啟</span>
                                    </template>
                                </el-table-column>
                                ...
                                <el-table-column header-align="center" align="center"  label="操作">
                                    <template slot-scope="scope">
                                        ...
                                        <el-button v-if="scope.row.lv == 1"
                                                size="mini"
                                                type="primary"
                                                @click="list_model(scope.$index, scope.row)">
                                            中分類
                                        </el-button>
                                        <el-button v-if="scope.row.lv == 2"
                                                   size="mini"
                                                   type="primary"
                                                   @click="list_model(scope.$index, scope.row)">
                                            小分類
...
<script>
    ...
    var vue_obj = new Vue({
        ...
        data: {
            ...
            loading: false,
            clean_model:{},
            history_name:"classify_list_{{$parent?$parent->id:0}}",
            api_get_tabel:"{{route('classify_list')}}",
            api_model_index:"{{route('classify_index')}}",
            api_model_edit:"{{route('classify_edit')}}",
        },
        created:function () {
