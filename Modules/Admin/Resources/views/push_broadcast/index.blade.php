                    ...
                    <div class="table-responsive">
                        <div class="my-el-table">
                            <el-table :data="table_data" stripe highlight-current-row border
                                ...
                                <el-table-column header-align="center" align="center" prop="receive_object" label="對象">
                                    <template slot-scope="scope">
                                        <template v-if="scope.row.push_object == 1">
                                            全體
                                        </template>
                                        <template v-else>
                                            指定
                                ...
                                <el-table-column header-align="center" align="center" prop="status" label="狀態">
                                    <template slot-scope="scope">
                                        <template v-if="scope.row.push_manner == 1">
                                            已發送
                                        </template>
                                        <template v-else>
                                            已排程
...
<script>
    ...
    var vue_obj = new Vue({
        ...
        data: {
            ...
            loading: false,
            clean_model: {},
            history_name: "push_broadcast_list",
            api_get_tabel: "{{route('push_broadcast_list')}}",
            api_model_index: "{{route('push_broadcast_index')}}",
            api_model_edit: "{{route('push_broadcast_edit')}}",
            {{--api_table_status: "{{route('push_broadcast_status')}}",--}}
            {{--api_model_delete: "{{route('push_broadcast_del')}}",--}}
